...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    try {
        $pdo = config::getConnexion();
        $stmtG = $pdo->prepare("SELECT id_garage, nom_garage FROM garage WHERE idRDV = :idRDV LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_GET['action'])) {
    ...
    try {
        ...
        if ($isEditPost) {
            ...
            // Mode ajout : tous les champs requis
    ...
    WHERE idRDV = :idRDV");
        ...
        $nomClient = '';
        try {
            $stmtCli = $pdo->prepare("SELECT nomclient FROM user WHERE id_client = :id");
        ...
        $estimation = $prediction = null;

        if ($isAjax && $isNewRdv) {
            $baseUrl = 'http://' . $_SERVER['HTTP_HOST'] . rtrim(dirname($_SERVER['SCRIPT_NAME']), '/') . '/';

            // === ESTIMATION ===
            $ch = curl_init($baseUrl . 'Estimertemps.php');
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
                'type_service' => $type_service,
                'kilometrage'  => $infoVeh['kilometrageV'] ?? 0,
                'marque'       => $infoVeh['marqueV'] ?? ''
            ]));
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_TIMEOUT, 10);
            $estResponse = curl_exec($ch);
            curl_close($ch);
            
            if ($estResponse) {
                $estimation = json_decode($estResponse, true);
            }

            // === PRÉDICTION ===
            $ch = curl_init($baseUrl . 'predict_next_rdv.php');
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
                'id_vehicule'  => $id_vehicule,
                'type_service' => $type_service,
            ...
            if ($predResponse) {
                $prediction = json_decode($predResponse, true);
                // Ajouter l'ID du client à la prédiction pour le bouton d'enregistrement
                if ($prediction && is_array($prediction)) {
                    $prediction['id_client'] = $id_client;
                    $prediction['id_vehicule'] = $id_vehicule;
                }
            }
        }
        ...
        // Stocker le message dans la session pour une redirection propre
        $_SESSION['rdv_success_message'] = $successMsg;
        header("Location: GestionVehicule.php");
        exit();
...
function renderForm($clients, $vehicules, $services, $isEdit, $rdv_a_modifier, $vehiculePreselectionne, $prefix) {
    ...
    try {
        ...
        $garages = $pdo2->query("SELECT id_garage, nom_garage FROM garage ORDER BY nom_garage")->fetchAll(PDO::FETCH_ASSOC);
...
(function(){
    ...
    function showPredictionCard(prediction, rdvData) {
        ...
        var html = `
            ...
            <div class="prediction-km">
                <strong>Source :</strong> ${rec.source === 'historique_vehicule' ? 'Historique véhicule 📊' : 'Moyenne globale 📈'}
                ${rec.kilometrage_estime ? '<div class="mt-2"><strong>Km estimé :</strong> ' + rec.kilometrage_estime.toLocaleString() + ' km</div>' : ''}
            </div>
    ...
    function showConfirmationCard(rdv, est) {
        ...
        setTimeout(function(){
            ...
            document.getElementById('m_src').textContent = est.source==='ia' ? '🤖 Estimation par IA Claude' : '📊 Estimation par règles métier';
            ...
            if (rdv.prediction && rdv.prediction.success && rdv.id_client && rdv.id_vehicule) {
                showPredictionCard(rdv.prediction, {
                    id_vehicule: rdv.id_vehicule,
                    id_client: rdv.id_client,
                    description: rdv.description,
                    heure_rdv: rdv.heure_rdv
                });
            } else if (rdv.prediction) {
                console.warn("Prédiction reçue mais incomplète:", rdv.prediction);
            }
        }, 350);
    }
    ...
    // En mode edit : submit direct du formulaire (pas de fetch pour éviter les conflits JS)
    document.getElementById('m_btn').addEventListener('click', function(){
        if(!validForm()) return;
        // Soumettre le formulaire directement vers traitementRDV.php
        var form = document.getElementById('m_form');
        form.method = 'POST';
        form.action = 'traitementRDV.php?id=<?= $isEdit ? (int)($rdv_a_modifier['idRDV'] ?? 0) : '' ?>';
        form.submit();
    });
})();
</script>
...
function showCardSA(rdv, est){
    ...
    setTimeout(function(){
        ...
        document.getElementById('sa_src').textContent = est.source==='ia' ? '🤖 Estimation par IA Claude' : '📊 Estimation par règles métier';
        ...
        if (rdv.prediction && rdv.prediction.success && rdv.id_client && rdv.id_vehicule) {
            showPredictionCardSA(rdv.prediction, {
                id_vehicule: rdv.id_vehicule,
                id_client: rdv.id_client,
                description: rdv.description
            });
