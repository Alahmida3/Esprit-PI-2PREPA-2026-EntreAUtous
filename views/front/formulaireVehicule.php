...
// Récupérer depuis GET (passé par GestionVehicule via fetch URL)
$idClient     = (int)($_GET['id']     ?? $_SESSION['client']['id_client'] ?? 0);
$nomClient    = $_GET['nom']          ?? $_SESSION['client']['nom']       ?? '';
$prenomClient = $_GET['prenom']       ?? $_SESSION['client']['prenom']    ?? '';
?>
    ...
    <div class="modal-body">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control bg-light fw-bold"
                   value="<?php echo htmlspecialchars(trim($nomClient . ' ' . $prenomClient)); ?>"
                   readonly>
            <input type="hidden" name="idClient"  value="<?php echo $idClient; ?>">
            <input type="hidden" name="nomClient" value="<?php echo htmlspecialchars($nomClient); ?>">
        </div>
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        const valid = vMarque() & vImmat() & vDate() & vKm() & vImage();
