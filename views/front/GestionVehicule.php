...
if (isset($_GET['delete_rdv'])) {
    $controllerRDV->delete((int)$_GET['delete_rdv']);
...
if (empty($nomClient) && $idClient > 0) {
    ...
    $stmt = $pdo->prepare("SELECT nom FROM client WHERE id_client = :id");
    $stmt->execute(['id' => $idClient]);
    $row       = $stmt->fetch(PDO::FETCH_ASSOC);
    $nomClient = $row ? $row['nom'] : '';
}
...
if (isset($_GET['delete_id'])) {
    try {
        ...
        header("Location: GestionVehicule.php?success=supprime");
        exit();
    } catch (Exception $e) {
...
try {
    $pdo = config::getConnexion();
$stmt = $pdo->query("SELECT id_client, nom as nomclient FROM client ORDER BY nom");
...
<img src="../../assets/img/<?php echo htmlspecialchars($vehicule['imageVoiture'] ?? 'default.jpg'); ?>"
                                 class="card-img-top"
...
// ✅ APRÈS
fetch('formulaireVehicule.php?id=<?php echo (int)($_SESSION["user_id"] ?? 0); ?>&nom=<?php echo urlencode($_SESSION["nom"] ?? ""); ?>&prenom=<?php echo urlencode($_SESSION["prenom"] ?? ""); ?>', {
                method: 'GET',
                headers: { 'X-Requested-With': 'XMLHttpRequest' }
            })
            .then(response => {
