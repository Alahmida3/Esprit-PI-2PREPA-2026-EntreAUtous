...
if (isset($_GET['action']) && $_GET['action'] === 'logout') {
    ...
    header("Location: /integration/user/views/front/login.php");
    exit();
}
...
if (isset($_GET['action']) && $_GET['action'] === 'delete_account') {
    ...
    header("Location: /integration/user/views/front/register.php?success=deleted");
    exit();
}
...
function register($pdo) {
    ...
    if (empty($nom) || empty($prenom) || !filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($pass) < 6) {
        header("Location: /integration/user/views/front/register.php?error=invalid_data");
        exit();
    }
    if (!preg_match('/^[0-9]{8}$/', $tel)) {
        header("Location: /integration/user/views/front/register.php?error=invalid_phone");
        exit();
    }
    $check = $pdo->prepare("SELECT id_client FROM client WHERE email = ?");
    $check->execute([$email]);
    if ($check->rowCount() > 0) {
        header("Location: /integration/user/views/front/register.php?error=email_exists");
        exit();
    }
    ...
    header("Location: /integration/user/views/front/login.php?success=registered" . $faceParam);
    exit();
}
...
function login($pdo) {
    ...
    if ($email === 'admin@garage.com' && $pass === 'admin123') {
        ...
        header("Location: /integration/user/views/back/admin.php");
        exit();
    }
    ...
    if ($user && password_verify($pass, $user['mot_de_passe'])) {
        ...
        session_regenerate_id(true);
        header("Location: /integration/user/views/front/home.php");
    } else {
        header("Location: /integration/user/views/front/login.php?error=failed");
    }
    exit();
}
...
function update_profile($pdo) {
    if (!isset($_SESSION['user_id'])) {
        header("Location: /integration/user/views/front/login.php"); exit();
    }
    $id     = (int)$_SESSION['user_id'];
    $prenom = trim($_POST['prenom']    ?? '');
    $nom    = trim($_POST['nom']       ?? '');
    ...
    if (!empty($tel) && !preg_match('/^[0-9]{8}$/', $tel)) {
        header("Location: /integration/user/views/front/profile.php?error=invalid_phone"); exit();
    }
    ...
    if ($ok) {
        ...
        header("Location: /integration/user/views/front/profile.php?success=updated");
    } else {
        header("Location: /integration/user/views/front/profile.php?error=update_failed");
    }
    exit();
}
...
function change_password($pdo) {
    ...
    if (!$row || !password_verify($old, $row['mot_de_passe'])) {
        header("Location: /integration/user/views/front/profile.php?error=wrong_password"); exit();
    }

    $hash = password_hash($new, PASSWORD_BCRYPT);
    $ok   = $pdo->prepare("UPDATE client SET mot_de_passe = ? WHERE id_client = ?")->execute([$hash, $id]);
    header("Location: /integration/user/views/front/profile.php?" . ($ok ? 'success=password' : 'error=password_failed'));
    exit();
}
